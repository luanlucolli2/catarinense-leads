...
/** 🔧 Normaliza classe exibida */
const normalizeClasseLabel = (classe: string | null | undefined): string | null => {
  ...
  // variações de "atendimento ia" -> "Atendimento IA"
  const iaVariants = ["atendimento ia", "atendimento-ia", "atendimentoia", "ia"];
  if (iaVariants.includes(raw)) return "Atendimento IA";

  // mantém original
  return String(classe);
};

/** 🎨 Escolhe a cor do badge pela classe normalizada */
const classeBadgeClass = (label: string | null) => {
  ...
  if (low === "atendimento ia") {
    // Ciano (visual tech p/ IA)
...
export const LeadsTable = ({
  ...
  const renderClasseBadge = (rawClasse: string | null | undefined) => {
    const label = normalizeClasseLabel(rawClasse);
    if (!label) {
      return (
        <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-700">
          {EMPTY}
    ...
    return (
      <span className={cn("inline-flex px-2 py-1 text-xs font-semibold rounded-full", classeBadgeClass(label))}>
        {label}
      </span>
    );
  };
    ...
    return sortedLeads.map((lead) => (
      ...
      >
        ...
        <td className="px-3 xl:px-6 py-4 whitespace-nowrap align-middle text-center min-w-[90px]">
          {renderClasseBadge(lead.telefones[0]?.classe)}
        </td>
