...
/* =========================================================
 * Utils de Visibilidade e Skeletons aprimorados
 * =======================================================*/
...
/**
 * Célula de Skeleton dinâmico (imita texto, badges e botões)
 */
const SkeletonCell = ({
  align = "left",
  w = "w-24",
  ...
  <td
    className={cn(
      "px-3 xl:px-6 py-4 align-middle",
      align === "center" ? "text-center" : align === "right" ? "text-right" : "text-left"
    )}
  >
    <Skeleton
      className={cn(
        type === "text" && "h-4 rounded",
        type === "badge" && "h-5 rounded-full",
        type === "button" && "h-8 rounded-md",
        w,
...
/**
 * Skeleton para listagem Mobile
 */
const MobileSkeletonCard = ({ idx }: { idx: number }) => {
  const titleWidths = ["w-3/4", "w-1/2", "w-2/3", "w-4/5"];
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-4 w-full">
      <div className="flex justify-between items-start gap-3">
        <div className="flex-1 space-y-2.5">
          <Skeleton className={cn("h-5 rounded", titleWidths[idx % 4])} />
...
type SortFieldFGTS =
  | "nome" | "cpf" | "consulta" | "saldo" | "libera" | "created_at" | "updated_at"
  | "data_atualizacao" | "data_nascimento" | "contratos" | "data_contrato_recente"
  | "vendedor" | "ultima_origem_cadastral" | "ultima_origem_higienizacao"
...
export const LeadsTableFGTS = ({
    ...
    const valueForSort = (x: ProcessedLeadFGTS) => {
      switch (sortField) {
        ...
        case "fgts_off_authorized":
          ...
          return 2;
        case "contratos": return x.contratos ?? -1;
        case "saldo": case "libera": case "vendedor": return (x as any)[sortField] ?? "";
        default: {
  ...
  const SortButton = ({
    ...
    field: SortFieldFGTS; children: React.ReactNode; align?: "left" | "center" | "right";
  }) => (
    <button
  ...
  const Th = ({ children, align = "left", minW = "min-w-[120px]" }: { children: React.ReactNode; align?: "left" | "center" | "right"; minW?: string; }) => (
    <th className={cn("px-3 xl:px-6 py-3 text-xs font-medium text-gray-500 tracking-wider", align === "center" ? "text-center" : align === "right" ? "text-right" : "text-left", minW)}>
      {children}
    </th>
  );
  ...
  const phonePairCols = (idx: 1 | 2 | 3 | 4) => [
    {
      id: `telefone_${idx}`, header: <Th minW="min-w-[110px]">Fone {idx}</Th>,
              ...
              {idx === 1 && lead.telefones.length > 1 && (
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent><p>Este lead possui mais telefones.</p></TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              )}
            </div>
    ...
    {
      id: `classe_${idx}`, header: <Th align="center" minW="min-w-[84px]">Classe {idx}</Th>,
      cell: (lead: ProcessedLeadFGTS) => {
  ...
  const cols = [
...

    {
      id: "fgts_off_authorized", header: <Th align="center" minW="min-w-[150px]"><SortButton field="fgts_off_authorized" align="center">Autorizado (FGTS OFF)</SortButton></Th>, cell: (lead: ProcessedLeadFGTS) => (<td className="px-3 xl:px-6 py-4 whitespace-nowrap align-middle text-center min-w-[150px]">
        {lead.fgts_off_authorized === true ? <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-emerald-400 text-white">Autorizado</span>
        : lead.fgts_off_authorized === false ? <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-amber-500 text-white">Não autorizado</span>
    ...
    },
    {
      id: "fgts_off_consultado_em", header: <Th align="center" minW="min-w-[150px]"><SortButton field="fgts_off_consultado_em" align="center">Data consulta (FGTS OFF)</SortButton></Th>, cell: (lead: ProcessedLeadFGTS) => (<td className="px-3 xl:px-6 py-4 whitespace-nowrap text-sm text-gray-900 align-middle text-center min-w-[150px]">
        {lead.fgts_off_consultado_em ? lead.fgts_off_consultado_em : <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-700">Não consultado</span>}
      </td>)
    },
    ...
    { id: "vendedor", header: <Th minW="min-w-[160px]"><SortButton field="vendedor">Vendedor</SortButton></Th>, cell: (lead: ProcessedLeadFGTS) => (<td className="px-3 xl:px-6 py-4 whitespace-nowrap text-sm text-gray-900 align-middle text-left min-w-[160px] max-w-[220px] truncate">{display(lead.vendedor)}</td>) },
    {
      id: "ultima_origem_cadastral", header: <Th align="center" minW="min-w-[130px]"><SortButton field="ultima_origem_cadastral" align="center">Última origem (cad.)</SortButton></Th>, cell: (lead: ProcessedLeadFGTS) => (<td className="px-3 xl:px-6 py-4 whitespace-nowrap text-sm text-gray-900 align-middle text-center min-w-[130px]">
        {lead.ultima_origem_cadastral ? <span className="inline-flex px-2 py-1 text-xs font-medium bg-purple-100 text-purple-800 rounded-full max-w-[160px] truncate mx-auto">{lead.ultima_origem_cadastral}</span>
        : <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-700">{EMPTY}</span>}
      </td>)
    },
    {
      id: "ultima_origem_higienizacao", header: <Th align="center" minW="min-w-[130px]"><SortButton field="ultima_origem_higienizacao" align="center">Última origem (hig.)</SortButton></Th>, cell: (lead: ProcessedLeadFGTS) => (<td className="px-3 xl:px-6 py-4 whitespace-nowrap text-sm text-gray-900 align-middle text-center min-w-[130px]">
        {lead.ultima_origem_higienizacao ? <span className="inline-flex px-2 py-1 text-xs font-medium bg-violet-100 text-violet-800 rounded-full max-w-[160px] truncate mx-auto">{lead.ultima_origem_higienizacao}</span>
        : <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-700">{EMPTY}</span>}
      </td>)
    },
  ] as const;
  ...
  const renderSkeleton = (idx: number) => {
    ...
    return (
      <tr className="hover:bg-gray-50" key={idx}>
        {visibleCols.map((c, i) => {
          if (c.id === "cpf") return <SkeletonCell key={i} w="w-28" align="left" />;
          ...
          if (["saldo", "libera", "contratos"].includes(c.id)) 
            return <SkeletonCell key={i} w={['w-16', 'w-20', 'w-14'][idx % 3]} align="right" />;

          if (c.id === "vendedor") return <SkeletonCell key={i} w={['w-24', 'w-32', 'w-28'][idx % 3]} align="left" />;

          return <SkeletonCell key={i} w="w-32" align="left" />;
        })}
  ...
  const FGTSCard = ({ lead }: { lead: ProcessedLeadFGTS }) => {
    ...
    const sectionDadosVisible = hasAnyVisible(["consulta", "saldo", "libera", "data_atualizacao", "contratos", "data_contrato_recente", "vendedor"]);
    const sectionOrigensVisible = hasAnyVisible(["ultima_origem_cadastral", "ultima_origem_higienizacao"]);
    const sectionFgtsOffVisible = hasAnyVisible(["fgts_off_authorized", "fgts_off_consultado_em"]);

    const AuthPill = () => lead.fgts_off_authorized === true ? (
      <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200">Autorizado</span>
    ) : lead.fgts_off_authorized === false ? (
      <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-amber-100 text-amber-800 border border-amber-200">Não autorizado</span>
    ) : (
      <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-700 border border-gray-200">Sem dados</span>
    );
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3 max-w-full">
        <div className="flex justify-between items-start gap-3">
          ...
          <Button onClick={() => { setSelectedLeadId(lead.id); setIsModalOpen(true); }} variant="outline" size="sm" className="shrink-0">
        ...
        {sectionFgtsOffVisible && <>
          ...
          <Section title="FGTS OFF" icon={Database}>
            ...
            {isVisible("fgts_off_consultado_em") && (
              <DataRow label="Data consulta" value={lead.fgts_off_consultado_em ? lead.fgts_off_consultado_em : "Não consultado"} />
            )}
          </Section>
        </>}
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden w-full max-w-full">
            ...
            <table className={cn("w-full", tableMinWidthFGTS)}>
              <thead className="bg-gray-50 sticky top-0 z-30">
                <tr>
                  ...
                  <th className={cn("px-3 xl:px-6 py-3 text-xs font-medium text-gray-500 tracking-wider text-center sticky right-0 z-40 bg-gray-50 border-l border-gray-200", ACTIONS_COL_WIDTH)}>
                    Ações
                  </th>
                </tr>
              </thead>
        ...
        <div className="lg:hidden space-y-4 p-4 max-w-full">
          {isLoading ? (
            ...
            leads.map((lead) => <FGTSCard key={lead.id} lead={lead} />)
          )}
        </div>
        ...
        <div className="bg-white px-4 lg:px-6 py-3 border-t border-gray-200 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Button onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages || isLoading || totalPages === 0} variant="outline" size="sm">
...
type SortFieldCLT =
  ...
  | "matricula" | "ultima_origem_cadastral" | "elegivel"
...
export const LeadsTableCLT = ({
  ...
  const sortedLeads = useMemo(() => {
    ...
    const valueForSort = (x: ProcessedLeadCLT, field: SortFieldCLT) => {
      switch (field) {
        ...
        case "politica_credito_aprovado": return x.politica_credito_aprovado === true ? 0 : x.politica_credito_aprovado === false ? 1 : 2;
        case "idade": case "meses_admissao": case "politica_credito_prazo_maximo_disponivel":
        case "qtd_emprestimos_ativos_suspensos": case "emprestimos_legados":
  ...
  const SortButton = ({ field, children, align = "left" }: { field: SortFieldCLT; children: React.ReactNode; align?: "left" | "center" | "right"; }) => (
  ...
  const Th = ({ children, align = "left" }: { children: React.ReactNode; align?: "left" | "center" | "right"; }) => (
    <th className={cn("px-3 xl:px-6 py-3 text-xs font-medium text-gray-500 tracking-wider", align === "center" ? "text-center" : align === "right" ? "text-right" : "text-left", "min-w-[120px]")}>{children}</th>
  );
    ...
    return (
      <tr className="hover:bg-gray-50" key={idx}>
        {visibleCols.map((c, i) => {
          ...
          if (["idade", "sexo", "meses_admissao", "qtd_emprestimos_ativos_suspensos", "emprestimos_legados", "politica_credito_prazo_maximo_disponivel"].includes(c.id))
             return <SkeletonCell key={i} w="w-10" align="center" />;
  ...
  const CLTCard = ({ lead }: { lead: ProcessedLeadCLT }) => {
    ...
    const sectionPoliticaCreditoVisible = hasAnyVisible(["politica_credito_aprovado", "politica_credito_mensagem", "politica_credito_valor_maximo_disponivel", "politica_credito_prazo_maximo_disponivel", "politica_credito_data_consulta", "politica_credito_tabela_aprovada"]);
    const sectionEmprestimosVisible = hasAnyVisible(["qtd_emprestimos_ativos_suspensos", "emprestimos_legados"]);
    const sectionOrigensVisible = hasAnyVisible(["ultima_origem_cadastral"]);

    const SituacaoBadge = () => lead.not_found ? <span className="inline-flex px-2 py-1 text-[11px] font-semibold rounded-full bg-gray-100 text-gray-800 border border-gray-200">Não encontrado</span> : lead.elegivel === true ? <span className="inline-flex px-2 py-1 text-[11px] font-semibold rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200">Elegível</span> : lead.elegivel === false ? <span className="inline-flex px-2 py-1 text-[11px] font-semibold rounded-full bg-rose-100 text-rose-800 border border-rose-200">Não elegível</span> : <span className="inline-flex px-2 py-1 text-[11px] font-semibold rounded-full bg-gray-100 text-gray-800 border border-gray-200">Sem dados</span>;
    const PoliticaCreditoBadge = () => lead.politica_credito_aprovado === true ? <span className="inline-flex px-2 py-1 text-[11px] font-semibold rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200">Aprovada</span> : lead.politica_credito_aprovado === false ? <span className="inline-flex px-2 py-1 text-[11px] font-semibold rounded-full bg-rose-100 text-rose-800 border border-rose-200">Reprovada</span> : <span className="inline-flex px-2 py-1 text-[11px] font-semibold rounded-full bg-gray-100 text-gray-800 border border-gray-200">Sem dados</span>;
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3 max-w-full">
        ...
        {sectionEmprestimosVisible && <>
          ...
          <Section title="Empréstimos" icon={FileText}>
            ...
            {isVisible("emprestimos_legados") && <DataRow label="Legados" value={lead.emprestimos_legados === 1 ? "Sim" : lead.emprestimos_legados === 0 ? "Não" : display(lead.emprestimos_legados)} />}
          </Section>
        </>}
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden w-full max-w-full">
        ...
        <div className="lg:hidden space-y-4 p-4 max-w-full">
          {isLoading ? (
            ...
            leads.map((lead) => <CLTCard key={lead.id} lead={lead} />)
          )}
        </div>
...
type SortFieldMercantil =
  | "cpf" | "nome" | "created_at" | "updated_at" | "data_nascimento" | "mercantil_status"
  | "mercantil_mensagem_erro" | "mercantil_data_hora_origem" | "mercantil_data_primeiro_vencimento"
  | "mercantil_valor_financiado" | "mercantil_valor_iof" | "mercantil_valor_emprestimo"
  | "mercantil_quantidade_parcelas" | "mercantil_valor_liberado" | "mercantil_taxa_juros_mes"
  | "mercantil_valor_parcela" | "ultima_origem_cadastral" | "ultima_origem_mercantil";
...
export const LeadsTableMercantil = ({
  ...
  const handleViewLead = (lead: ProcessedLeadMercantil) => { setSelectedLeadId(lead.id); setIsModalOpen(true); };
  const handleCloseModal = () => { setIsModalOpen(false); setSelectedLeadId(null); };
  ...
  const sortedLeads = useMemo(() => {
    ...
    const valueForSort = (x: ProcessedLeadMercantil, field: SortFieldMercantil) => {
      switch (field) {
        case "created_at": case "updated_at": case "data_nascimento":
        case "mercantil_data_hora_origem": case "mercantil_data_primeiro_vencimento":
          return (x as any)[field] ? new Date((x as any)[field]).getTime() : Number.POSITIVE_INFINITY;
  ...
  const SortButton = ({ field, children, align = "left" }: { field: SortFieldMercantil; children: React.ReactNode; align?: "left" | "center" | "right"; }) => (
    <button onClick={() => handleSort(field)} className={cn("flex items-center gap-1 hover:bg-gray-100 px-2 py-1 rounded transition-colors duration-150 w-full", align === "center" && "justify-center", align === "right" && "justify-end")}>
      <span>{children}</span>
      {sortField === field ? (sortDirection === "asc" ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />) : <div className="w-3 h-3" />}
    </button>
  ...
  const phonePairCols = (idx: 1 | 2 | 3 | 4) => [
    { id: `telefone_${idx}`, header: <Th>Fone {idx}</Th>, cell: (lead: ProcessedLeadMercantil) => (<td className="px-3 xl:px-6 py-4 whitespace-nowrap text-left min-w-[110px]">{lead.telefones[idx - 1]?.fone ? <span className="font-mono">{display(lead.telefones[idx - 1]?.fone)}</span> : EMPTY}</td>) },
    { id: `classe_${idx}`, header: <Th align="center">Classe {idx}</Th>, cell: (lead: ProcessedLeadMercantil) => { const c = getClasseBadge(lead.telefones[idx - 1]?.classe); return <td className="px-3 xl:px-6 py-4 whitespace-nowrap text-center min-w-[84px]"><span className={cn("inline-flex px-2 py-1 text-xs font-semibold rounded-full", c.cls)}>{c.label}</span></td> } }
  ] as const;
  ...
  const cols = [
    ...
    { id: "mercantil_status", header: <Th align="center"><SortButton field="mercantil_status" align="center">Status</SortButton></Th>, cell: (lead: ProcessedLeadMercantil) => (<td className="px-3 xl:px-6 py-4 whitespace-nowrap text-center min-w-[130px]">{lead.mercantil_status === "SUCESSO" ? <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-emerald-500 text-white">Sucesso</span> : lead.mercantil_status && lead.mercantil_status !== EMPTY ? <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-amber-500 text-white">{lead.mercantil_status}</span> : <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-700">Sem dados</span>}</td>) },
    { id: "mercantil_data_hora_origem", header: <Th align="center"><SortButton field="mercantil_data_hora_origem" align="center">Data/hora consulta</SortButton></Th>, cell: (lead: ProcessedLeadMercantil) => (<td className="px-3 xl:px-6 py-4 whitespace-nowrap text-center min-w-[150px]">{lead.mercantil_data_hora_origem ? <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">{lead.mercantil_data_hora_origem}</span> : <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-700">Ainda não consultado</span>}</td>) },
    ...
    { id: "ultima_origem_cadastral", header: <Th align="center"><SortButton field="ultima_origem_cadastral" align="center">Última origem (cad.)</SortButton></Th>, cell: (lead: ProcessedLeadMercantil) => (<td className="px-3 xl:px-6 py-4 whitespace-nowrap text-center min-w-[130px]">{lead.ultima_origem_cadastral ? <span className="inline-flex px-2 py-1 text-xs font-medium bg-purple-100 text-purple-800 rounded-full max-w-[160px] truncate mx-auto">{lead.ultima_origem_cadastral}</span> : <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-700">{EMPTY}</span>}</td>) },
    { id: "ultima_origem_mercantil", header: <Th align="center"><SortButton field="ultima_origem_mercantil" align="center">Última origem (Merc.)</SortButton></Th>, cell: (lead: ProcessedLeadMercantil) => (<td className="px-3 xl:px-6 py-4 whitespace-nowrap text-center min-w-[130px]">{lead.ultima_origem_mercantil ? <span className="inline-flex px-2 py-1 text-xs font-medium bg-indigo-100 text-indigo-800 rounded-full max-w-[160px] truncate mx-auto">{lead.ultima_origem_mercantil}</span> : <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-700">{EMPTY}</span>}</td>) },
  ] as const;
    ...
    return (
      <tr className="hover:bg-gray-50" key={idx}>
        {visibleCols.map((c, i) => {
          if (c.id === "cpf") return <SkeletonCell key={i} w="w-24" align="left" />;
          if (c.id === "nome") return <SkeletonCell key={i} w={nameWidths[idx % 5]} align="left" />;
          if (["data_nascimento", "created_at", "updated_at", "mercantil_data_hora_origem", "mercantil_data_primeiro_vencimento"].includes(c.id))
            return <SkeletonCell key={i} w="w-24" align="center" />;
            
          if (["mercantil_status", "ultima_origem_cadastral", "ultima_origem_mercantil", "classe_1", "classe_2", "classe_3", "classe_4"].includes(c.id))
             return <SkeletonCell key={i} w="w-20" align="center" type="badge" />;
             
          if (["mercantil_valor_financiado", "mercantil_valor_iof", "mercantil_valor_emprestimo", "mercantil_valor_liberado", "mercantil_valor_parcela"].includes(c.id))
            return <SkeletonCell key={i} w={['w-16', 'w-20', 'w-24'][idx % 3]} align="right" />;
            
          if (["mercantil_quantidade_parcelas", "mercantil_taxa_juros_mes"].includes(c.id)) 
  ...
  const MercantilCard = ({ lead }: { lead: ProcessedLeadMercantil }) => {
    ...
    const sectionTelefonesVisible = hasAnyVisible(["telefone_1", "telefone_2", "telefone_3", "telefone_4", "classe_1", "classe_2", "classe_3", "classe_4"]);
    const sectionStatusVisible = hasAnyVisible(["mercantil_status", "mercantil_mensagem_erro", "mercantil_data_hora_origem"]);
    const sectionFinanceiroVisible = hasAnyVisible(["mercantil_valor_emprestimo", "mercantil_valor_iof", "mercantil_valor_financiado", "mercantil_data_primeiro_vencimento", "mercantil_quantidade_parcelas", "mercantil_valor_liberado", "mercantil_taxa_juros_mes", "mercantil_valor_parcela"]);
    const sectionRegistroVisible = hasAnyVisible(["ultima_origem_cadastral", "ultima_origem_mercantil"]);
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden w-full max-w-full">
        ...
        <div className="lg:hidden space-y-4 p-4 max-w-full">
          {isLoading ? (
            ...
            leads.map((lead) => <MercantilCard key={lead.id} lead={lead} />)
          )}
        </div>
