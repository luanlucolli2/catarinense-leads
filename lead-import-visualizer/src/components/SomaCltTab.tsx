  ...
  return <Dialog open={isOpen} onOpenChange={(open) => { if (!open && !submitting) { reset(); onClose(); } }}>
    <DialogContent className="max-w-2xl">
      ...
      <div className="space-y-4 py-4">
        ...
        <div className="space-y-2"><Label htmlFor="soma-lines" className="text-sm font-medium">Linhas (CPF e Nome completo) *</Label><Textarea id="soma-lines" value={lines} onChange={(event) => setLines(event.target.value)} placeholder={'700367136\tRICARDO MENDES FIGUEIRA\n12345678909;MARIA SILVA'} className={cn("min-h-[200px] w-full font-mono text-sm", noFocus)} disabled={submitting} />
          <div className="flex flex-col gap-2 text-xs text-gray-600 sm:flex-row sm:items-center sm:justify-between"><span>Use CPF;NOME ou CPF + tabulação + NOME. CPFs recebem zeros à esquerda.</span><span className="font-medium text-blue-600">Detectadas: {lineCount.toLocaleString()} linhas</span></div></div>
