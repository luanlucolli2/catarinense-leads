...
const WIZARD_STEPS = [{ label: "Selecionar leads", icon: Users }, { label: "Configurar disparo", icon: Settings2 }, { label: "Confirmar", icon: Check }]
...
const TEMPLATE_QUALITY_LABELS: Record<OfficialTemplate["quality"], { label: string; className: string }> = {
...
export default function DisparosWhatsappVendeaiPage() {
  ...
  const canContinueFromStepOne = configuration.product !== "" && (source === "pasted_numbers" ? phoneListSummary.validNumbers.length > 0 && phoneListSummary.invalidTokens.length === 0 : filters.selectedBanks.length > 0 && bankErrors.length === 0 && registeredPreview.status === "ready" && recipientCount > 0)
...
function SenderConfigurationCard({ inbox, sender, recipientCount, onToggleTemplate, onUpdateSender }: { inbox: OfficialInbox; sender: SenderConfiguration; recipientCount: number; onToggleTemplate: (template: OfficialTemplate) => void; onUpdateSender: (updater: (sender: SenderConfiguration) => SenderConfiguration) => void }) {
  const quality = QUALITY_LABELS[inbox.qualityRating]
