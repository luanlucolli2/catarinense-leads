...
class StoreFactaCltExternalReportJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function handle(FactaCltExternalApiService $api): void
    {
        $job = FactaCltConsultJob::query()->find($this->jobId);
        ...
        if (! $response->successful()) {
            throw new \RuntimeException('Relatório externo Facta CLT indisponível.');
        }

        $diskName = (string) config('facta.storage.reports_disk', 'local');
        $disk = Storage::disk($diskName);
        $directory = (string) config('facta.storage.dir_reports', 'facta-clt-reports');
        ...
        if (! $disk->exists($path)) {
            throw new \RuntimeException('Falha ao armazenar relatório externo Facta CLT.');
        }
